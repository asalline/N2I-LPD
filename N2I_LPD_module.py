...
### ORIGINAL FUNCTION
def combine_sinogram(split_sinogram: torch.Tensor,
                    num_of_splits: int,
...
### ORIGINAL FORWARD FUNCTION
    def forward(self, f, g, g_expand, h, operator_split, fbp_list, domain, num_of_splits, averaged, device='cuda'):
        ...
        ### Line number 3 in pseudocode {
        for j in range(f.shape[1]):
            f_sinograms[0,j,:,:] = operator_split[j](f[:,j,:,:])
        ### }
        
        #### Line number 4 in pseudocode {
        with torch.no_grad():
            sino_reco = combine_sinogram(f_sinograms, num_of_splits, device=device)
        ### }

        ### Line number 5 in pseudocode {
        u[:,:,:] = torch.cat([h[None,None,:,:], sino_reco[None,None,:,:], g_expand[None,None,:,:]], dim=1)
        h[:,:] = h[:,:] + self.dual_resnet(u[None,:,:,:])[0,0,:,:].to(device)
        ### }
        
        ### Line number 6&7 in pseudocode {
        for j in range(g.shape[1]):
            adjoint_eval[j,:,:] = fbp_list[j](h[j::int(g.shape[1]),:][None,None,:,:])
        ### }
        
        ### Line number 8 in pseudocode {
        adjoint_eval = torch.mean(adjoint_eval, dim=0)
        ### }
        
        ### Line number 9 in pseudocode {
        u = torch.zeros((2, ) + (f.shape[2], f.shape[3])).to(device)
        f = torch.mean(f, dim=1)
        u[:,:,:] = torch.cat([f, adjoint_eval[None,:,:]], dim=0)
        f[0,:,:] = f[0,:,:] + self.primal_resnet(u[None,:,:,:])[0,0,:,:].to(device)
        ### }
...
class LearnedPrimalDual(nn.Module):
    ...
    def forward(self, input_tensor: torch.Tensor,
        ...
        ### Initialize g' and h_0
        input_tensor_2_expand = combine_sinogram(input_tensor_2,num_of_splits,device=device)
